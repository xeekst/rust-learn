...
pub fn exec_cmd_nowindow_wait_output(cmd_with_args: &str) -> Result<Vec<u8>> {
    let child = Command::new("cmd")
        .arg("/c")
        .creation_flags(WINDOWS_CREATE_NO_WINDOW)
        .raw_arg(format!("{}", cmd_with_args))
        .output()?;
...
pub fn exec_cmd_nowindow_wait_output_use_codepage_encoding(cmd_with_args: &str) -> Result<String> {
    ...
    let codepage = String::from_utf8(codepage_buf)?.replace("Active code page: ", "").replace("\r\n", "");
    let codepage = codepage.parse::<u16>()?;
    let encoding = match to_encoding(codepage) {
        Some(d) => d,
        None => {
            error!("codepage:{codepage} not support, please add codepage map encoding");
            return Err(anyhow::anyhow!("codepage:{codepage} not support, please add codepage map encoding"));
        }
    };
    let (text, enc, is_success) = encoding.decode(&buf);
    if !is_success {
