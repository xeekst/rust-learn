...
fn main() {
    // let e = to_encoding(936 as u16).unwrap();
    // let buf = get_graphics_cards();
    // let (s, enc, b) = e.decode(&buf);
    // println!("s:{:?},enc:{:?},b:{:?}", s, enc, b);
	get_graphics_cards2();
...
pub fn get_graphics_cards() -> Vec<u8> {
    system_process::exec_cmd_nowindow_wait_output("wmic bios get serialnumber")
        .unwrap()
}

pub fn get_graphics_cards2() {
    let s = system_process::exec_cmd_nowindow_wait_output_use_codepage_encoding("wmic bios get serialnumber")
        .unwrap();
	println!("s:{s}");
}
...
static CODE_PAGES: [u16; 52] = [
    ...
    28601, 50220, 50222, 50225, 50227, 51936, 51949, 52936, 850,
];

/// Encodings corresponding to the code page numbers in the same order
static ENCODINGS: [&'static Encoding; 52] = [
    &UTF_8_INIT,
    &UTF_16LE_INIT,
    &WINDOWS_1252_INIT,
    &WINDOWS_1251_INIT,
